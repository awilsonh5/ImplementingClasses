...
class Point(object):
    ...
    def move_to(self, x, y):
        ...
        distance = ((self.x - a) ** 2 + (self.y - b) ** 2) ** .5
    ...
    def move_by(self, dx, dy):
        ...
        self.y = self.y + dy
        self.number_of_moves_made = self.number_of_moves_made + 1
        distance = ((self.x - (self.x - dx)) ** 2 + (self.y - (self.y - dy)) ** 2) ** .5
        self.distance_traveled = self.distance_traveled + distance
    ...
    def get_distance_from(self, point):
        distance_from_point = ((point.x - self.x)**2 + (point.y - self.y)**2)**.5
        return distance_from_point

    def get_distance_from_start(self):
        distance_from_start = ((self.x - self.startx) ** 2 + (self.y - self.starty) ** 2) ** .5
    ...
    def closer_to(self, p2, p3):
        distance_from_p2 = ((p2.x - self.x) ** 2 + (p2.y - self.y) ** 2) ** .5
        distance_from_p3 = ((p3.x - self.x) ** 2 + (p3.y - self.y) ** 2) ** .5
    ...
    def halfway_to(self, p2):
        halfway_point = Point((p2.x + self.x)//2, (p2.y + self.y)//2)
